...
const Invoice = React.forwardRef(({ workOrder, className }, ref) => {
  ...
  return (
    <div
      ref={ref}
      className={`max-w-6xl mx-auto p-4 md:p-6 bg-white border border-gray-300 ${
        className || ""
      }`}
    >
      {/* Header Section */}
        ...
        <div className="text-left md:text-right border border-gray-300 p-3">
          ...
          <div>
            <div className="font-semibold">
              Tax Invoice no: TLCP/{workOrder.jobAddress.jobNumber}
            </div>
          </div>
        </div>
      </div>
        ...
        <table className="w-full border-collapse text-xs">
          ...
          <thead>
            <tr className="border-b border-gray-300 bg-gray-50">
              ...
              <th className="text-right py-2 px-4 font-semibold border-r border-gray-300">
                Quantity
              </th>
              <th className="text-right py-2 px-4 font-semibold border-r border-gray-300">
                Unit Price
          ...
          <tbody>
            {workOrder.jobDetails.workItems
              ...
              .map((item, index) => (
                <tr key={index} className="border-b border-gray-200">
                  <td className="py-1 px-4 border-r border-gray-300">
                    {item.itemNumber}
                  </td>
                  <td className="py-1 px-4 border-r border-gray-300">
                    {item.description}
                  </td>
                  <td className="py-1 px-4 text-right border-r border-gray-300">
                    {item.quantity}
                  </td>
                  <td className="py-1 px-4 text-right border-r border-gray-300">
            ...
            <tr className="border-t-2 border-gray-300 bg-gray-50">
              ...
              <td className="py-1 px-4 text-right font-semibold border-r border-gray-300">
                Sub Total:
              </td>
      ...
      <div className="border-t border-gray-300 pt-6 banking-details-container">
        <div className="font-bold text-lg mb-4 text-start">BANKING DETAILS</div>
        <div className="grid grid-cols-[max-content_1fr] gap-x-4 gap-y-1 text-sm text-start banking-details-content">
